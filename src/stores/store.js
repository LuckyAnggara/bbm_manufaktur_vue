    ...
    async getBisnisData() {
      ...
      try {
        const response = await axiosIns.get(
          `/dashboard/bisnis?data-sales=${this.filter.dataSales}&data-purchasing=${this.filter.dataPurchasing}`
        )
...
export const useItemStore = defineStore('itemStore', {
  ...
  actions: {
    async showLaporanProduksi() {
      this.isLoadingDownload = true
      try {
        const response = await axiosIns.get(`/report/persediaan/produksi`)
...
export const useProductionOrderStore = defineStore('productionOrderStore', {
  ...
  actions: {
    ...
    async getAllData(page = '') {
      ...
      try {
        const response = await axiosIns.get(
          `/production-order?limit=${this.currentLimit}${this.searchQuery}${this.fromToDate}${this.itemByTipe}`
        )
        this.responseListData = response.data.data
      } catch (error) {
        Swal.fire({
